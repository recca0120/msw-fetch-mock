...
export function createFetchMock(externalServer?: SetupServerLike): FetchMock {
  let server: SetupServerLike | null = externalServer ?? null;
  const ownsServer = !externalServer;
  let interceptors: PendingInterceptor[] = [];
  ...
  return {
    ...
    disableNetConnect() {
      // MSW with onUnhandledRequest: 'error' already blocks unhandled requests
    },
    ...
    assertNoPendingInterceptors() {
      const unconsumed = interceptors.filter(isPending);
      interceptors = [];
      if (ownsServer) {
    ...
    pendingInterceptors(): PendingInterceptor[] {
      return interceptors.filter(isPending).map((p) => ({ ...p }));
    },
            ...
            reply(
              ...
              const handler = getHttpMethod(method)(
                ...
                async ({ request }: { request: StrictRequest<DefaultBodyType> }) => {
                  ...
                  recordCall(callHistory, request, bodyText);
